...
if ($conn->connect_error) {
  wh_log("Connection failed: " . $conn->connect_error);
  die("Connection failed: " . $conn->connect_error);
}

//logging

wh_log("messagetype = ".$type);
wh_log("lastmessage = ".$lastmessage);
...
if ($row = $result->fetch_assoc()) {
      ...
      if (( $lastseen==0 ) || ($lastseen <= (time()-60))) {
		...
		$conn->query($sql);
		wh_log("Delete old messages (user joined the chat)");
        ...
        $conn->query($sql);
        wh_log("insert messages (user joined the chat)");
...
if (strlen($version) > 0) {
	...
	$conn->query($sql);
	wh_log("Update ESP32 version ".$version);
}
// =================================================================================================================
...
if (strlen($eeprom) > 0) {
	...
	$conn->query($sql);
	wh_log("Update eeprom version ".$eeprom);
}
// =================================================================================================================
...
// select messages
  if ($type==0){
      ...
      $result = $conn->query($sql);
	  if($row = $result->fetch_assoc()){
			wh_log("Jump outputmessage ".$row["rowid"]);
			outputMessage($row["rowid"]);
			wh_log("Return outputmessage ".$row["rowid"]);
	  } else{
			...
			//wh_log("No more public messages");
			wh_log('{"rowid":"'.$lastmessage.'","timestamp":"0","message":"0","nickname":"0","len":0,"pm":'.$pm.'}');
	  }	  
	}
  else {
		// private message
		...
		if($row = $result->fetch_assoc()){
			...
			wh_log('{"rowid":"'.$lastprivate.'","timestamp":"0","message":"0","nickname":"0","len":0,"pm":0}');
		}       
	   }
echo"<br><br>";
