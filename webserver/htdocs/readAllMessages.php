...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  writeLog("lastprivate     =" . $lastprivate);      
  writeLog("previousPrivate =" . $previousPrivate);      
...
if (!str_contains('bodger',$theNickName)) updateLastSeen($regid);

// Insert a message <Nickname> has joined the chat
if (intval($lastmessage) <= 1) {
	 
	if (!str_contains('testsmd,testtht,testzx,testatari,bodger,tinker',$theNickName)) insertWelcomeMessage($regid);
}
...
function insertWelcomeMessage($regid) {
    ...
    //  $dt = date('d-m-Y h:i:s');
    //  $sql = "insert INTO tempUsers (Nickname,Mac,Regid,dateTime) VALUES ('$nickname','$macaddress','$regid','$dt')";
    //  $conn->query($sql); 
    // }
}
// =================================================================================================================
